...
vec3 sampleGradient(float t, sampler2D gradTex){
    t = clamp(t, 0.0, 1.0);
    return texture(gradTex, vec2(t, 0.5)).rgb;
}
...
void main(){
    ...
    for (int i = 0; i < 44; ++i) {
      ...
      Pl = hoverMat * Pl;
        ...
        vec3 userGradient = 2.0 * sampleGradient(tRay, uGradient);
...
const PrismaticBurst = ({
  ...
  useEffect(() => {
    ...
    if (canvas) {
      canvas.style.mixBlendMode = (mixBlendMode && mixBlendMode !== 'none' ? mixBlendMode : '') as any;
    }
  }, [mixBlendMode]);
  ...
  useEffect(() => {
    ...
    program.uniforms.uAnimType.value = (animTypeMap as any)[animationType ?? 'rotate'];
    ...
    if (color0) finalColors[0] = color0;
    if (color1) finalColors[1] = color1;
    if (color2) finalColors[2] = color2;

    let count = 0;
    if (finalColors.length > 0) {
      const gl = renderer.gl;
      const capped = finalColors.slice(0, 64);
      count = capped.length;
  ...
  }, [intensity, speed, animationType, colors, distort, offset, rayCount, color0, color1, color2]);
